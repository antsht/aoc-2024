        ...
        for (size_t inner_row = row; inner_row < max_row; ++inner_row) {
          for (size_t inner_col = 0; inner_col < max_col; ++inner_col) {
            ...
            if (data[inner_row][inner_col] == current) {
              dataNew[row][col] = '#';
              dataNew[inner_row][inner_col] = '#';

              int new_r1, new_c1, new_r2, new_c2;
              int r_diff = inner_row - row;
              int c_diff = inner_col - col;
              new_r1 = row - r_diff;
              new_c1 = col - c_diff;
              while (new_r1 >= 0 && new_r1 < static_cast<int>(max_row) &&
                  new_c1 >= 0 && new_c1 < static_cast<int>(max_col)) {
                dataNew[new_r1][new_c1] = '#';
                new_r1 = new_r1 - r_diff;
                new_c1 = new_c1 - c_diff;
              }

              new_r2 = inner_row + r_diff;
              new_c2 = inner_col + c_diff;

              while  (new_r2 >= 0 && new_r2 < static_cast<int>(max_row) &&
                  new_c2 >= 0 && new_c2 < static_cast<int> (max_col)) {
                dataNew[new_r2][new_c2] = '#';

                new_r2 = new_r2 + r_diff;
                new_c2 = new_c2 + c_diff;
              }
            }
          }
        }
