...
bool checkDataRowValidity(std::vector<int> &dataRow, const std::unordered_multimap<int, int> &rules);
...
for (auto& row : data) {
  if( !checkDataRowValidity(row, rules)){
    std::cout << "Row is not valid: "<<row[row.size() / 2]<< std::endl;
